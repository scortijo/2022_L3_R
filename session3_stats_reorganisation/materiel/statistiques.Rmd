...
Imaginons que nous développons un traitement qui est sensé modifier la biomasse des plantes. Nous souhaitons savoir s'il est efficace ou non. Nous mettons donc en place une expérience dans laquelle nous exposons deux groupes de plantes à des conditions contrôlées, de manière à ce que la seule différence entre les deux groupes soit l’application du traitement ou l'application d'un contrôle. 

Sur les graphes ci-dessous, il est difficile de dire si les traitements ont un réel effet ou si les différences de moyennes sont uniquement dues à l'aléa, et à la variabilité du phénotype de biomasse entre les plantes :
...
set.seed(116)
N <- 15
biomasse <- data.frame(contrôle = rnorm(n = N, mean = 10, sd = 2),
                       traitement = rnorm(n = N, mean = 14, sd = 2)) %>%
  pivot_longer(c(contrôle, traitement), names_to = "Traitement", values_to = "Biomasse")

ggplot(biomasse, aes(x=Traitement, y=Biomasse)) + 
  geom_boxplot(color="darkblue") + xlab("")+
  geom_jitter(size=3)+theme_pubr() + ggtitle("Traitement 1")

biomasse_no_effect <- data.frame(contrôle = rnorm(n = N, mean = 10, sd = 2),
...
ggplot(biomasse_no_effect, aes(x=Traitement, y=Biomasse)) + 
  ...
  theme_pubr()+ ggtitle("Traitement 2")
```

Nous pouvons donc tester si l'effet d'un traitement sur la biomasse est **significatif** au moyen d'un test statistique.
...
+ **Calculer une statistique de test observée : ** il s'agit d'une grandeur calculée sur les données, par exemple la différence de moyenne entre notre groupe traitement et notre groupe contrôle.

+ **Comparer notre statistique de test observée à la distribution de cette statistique sous l'hypothèse nulle : ** dans notre cas, il faut comparer la différence de moyennes entre traitement et contrôle à la distribution des écarts de moyennes attendus sous l'hypothèse nulle (une distribution centrée en 0 car pas d'effet traitement). Cela nous permet de calculer une **p-value** : la probabilité d'observer des données au moins aussi extrêmes que les nôtres sous l'hypothèse nulle. Dans notre exemple, il s'agit de la probabilité d'observer, sous l'hypothèse que le traitement n'a pas d'effet, une telle différence de moyennes.

+ **Conclure : ** Si la p-value est très faible alors c'est que nos données sont très improbables sous l'hypothèse nulle, et donc qu'on peut se permettre de rejeter l'hypothèse nulle.  Pour déterminer à partir de quand nous jugeons nos données suffisamment "improbables", on fixe au préalable un seuil à notre test, traditionnellement 5%. Ce seuil établit que si on a moins de 5% de chances d'observer des données aussi extrêmes que les nôtres sous l'hypothèse nulle, alors on peut rejeter cette hypothèse. Dans notre cas, nous pourrons conclure à une efficacité **significative** du traitement sur la biomasse si la p-value du test est inférieure au seuil fixé.

Visualisons sur des graphes le calcul des p-values, via les valeurs observées de différences de moyennes de 2 cas. Ces différences de moyennes observées sont comparées à la distribution des écarts de moyennes sous l'hypothèse nulle (pas d'effet traitement) : 
...
density <- ggplot(data = df, aes(x=h0)) + geom_density(fill="grey", alpha = 0.2) + 
  theme_pubr() + xlab("Différence de moyennes") +xlab("Fréquence") +
  geom_vline(xintercept = observed_effect, color="darkblue", size=2)+ 
  ggtitle("Statistique de test du traitement 1 comparée à la distribution nulle")
...
La p-value correspond à l'aire colorée (la probabilité d'avoir une différence de moyenne au moins aussi extrême que nos données par rapport à l'hypothèse nulle). Elle est très faible, inférieure à 5%, car l'écart de moyennes important du cas 1 est assez extraordinaire sous l'hypothèse de non efficacité du traitement.
...
density <- ggplot(data = df, aes(x=h0)) + geom_density(fill="grey", alpha = 0.2) + 
  ...
  ggtitle("Statistique de test du traitement 2 comparée à la distribution nulle")
...
**Le traitement 1 montre une différence significative due à l'effet du traitement sur la biomasse, alors que le traitement 2 non.**
...
**Attention : significativité d'un test de comparaison de moyennes ou d'un test de corrélation ne signifie pas qu'il y a causalité entre les deux variables étudiées. Il faut être précautionneux et raisonnable dans les interprétation en fonction de votre plan expérimental.**
