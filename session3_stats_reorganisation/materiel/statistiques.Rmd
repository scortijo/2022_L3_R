...
En effet, une observation "à l'oeil" des données ou d'un graphique n'est pas une approche objective, reproductible, ni rigoureuse, pour faire des interprêtations biologiques. 
...
Sur ces graphes par exemple, il est difficle de dire si le traitement a un réel effet ou si les différences sont uniquement dues à l'éléa, et à la variabilité du phénotype de biomasse entre les plantes :
...
set.seed(116)
N <- 15
biomasse <- data.frame(controle = rnorm(n = N, mean = 10, sd = 2),
                       traitement = rnorm(n = N, mean = 14, sd = 2)) %>%
  pivot_longer(c(controle, traitement), names_to = "Traitement", values_to = "Biomasse")

ggplot(biomasse, aes(x=Traitement, y=Biomasse)) + 
  geom_boxplot(color="darkblue") + xlab("")+
  geom_jitter(size=3)+theme_pubr() + ggtitle("Cas 1")

biomasse_no_effect <- data.frame(controle = rnorm(n = N, mean = 10, sd = 2),
                       traitement = rnorm(n = N, mean = 10, sd = 2)) %>%
  pivot_longer(c(controle, traitement), names_to = "Traitement", values_to = "Biomasse")

ggplot(biomasse_no_effect, aes(x=Traitement, y=Biomasse)) + 
  geom_boxplot(color="darkred") + 
  geom_jitter(size=3)+xlab("")+
  theme_pubr()+ ggtitle("Cas 2")
```

Nous pouvons donc tester si l'effet de notre traitement sur la biomasse est **significatif** au moyen d'un test statistique.

Un test statistique se déroule gloablement en 4 étapes : 

+ **Définir une hypothèse nulle : ** cette hypothèse représente souvent l'aléatoire, l'absence d'effet d'un traitement, l'absence de différence entre deux groupes, on la qualifie parfois de l'hypothèse "ennuyeuse". Dans notre cas, il s'agit d'une égalité de moyennes entre nos deux groupes, nous nous plaçons dans le cas ou le traitement n'a pas d'effet sur la biomasse.

+ **Calculer une statistique de test observée : ** il s'agit d'une grandeur calculée sur les données, par exemple la différence de moyenne entre notre groupe traitement et notre groupe controle.

+ **Comparer notre statistique de test observée à la distribution de cette statistique sous l'hypoèse nulle : ** dans notre cas, il faut comparer la différence de moyennes entre traitement et controle à la distribution des écarts de moyennes attendus sous l'hypothèse nulle (une distribution centrée en 0 car pas d'effet traitement). Cela nous permet de calculer une **p-value** : la probabilité d'observer des données au moins aussi extrèmes que les notres sous l'hypothèse nulle. Dans notre exemple, il s'agit de la probabilité d'observer, sous l'hypothèse que le traitement n'a pas d'effet, une telle différence de moyennes.

+ **Conclure : ** Si la p-value est très faible alors c'est que nos données sont très improbables sous l'hypothèse nulle, et donc qu'on peut se permettre de rejeter l'hypothèse nulle.  Pour déterminer à partir de quand nous jugeons nos données suffisamment "improbables", on fixe au préalable un seuil à notre test, traditionnellent 5%. Ce seuil établit que si on a moins de 5% de chances d'observer des données aussi extrèmes que les notres sous l'hypothèse nulle, alors on peut rejeter cette hypothèse. Dans notre cas, nous pourrons conclure à une efficacité **significative** du traitement sur la biomasse si la p-value du test est inférieure au seuil fixé.

Visualisaons sur des graphes le calcul des p-values, via les valeurs observées de différences de moyennes de 2 cas. Ces différences de moyennes observées sont comparées à la distribution des écarts de moyennes sous l'hypothèse nulle (pas d'effet traitement) : 

```{r, fig.height=3, echo=FALSE}
observed_effect <- mean(biomasse[biomasse$Traitement=="traitement",]$Biomasse - biomasse[biomasse$Traitement!="traitement", ]$Biomasse)
observed_no_effect <- mean(biomasse_no_effect[biomasse_no_effect$Traitement=="traitement",]$Biomasse - biomasse_no_effect[biomasse_no_effect$Traitement!="traitement", ]$Biomasse)
...
density <- ggplot(data = df, aes(x=h0)) + geom_density(fill="grey", alpha = 0.2) + 
  ...
  ggtitle("Statistique de tests du cas 1 comparée à la distribution nulle")
...
La p-value correspond à l'aire colorée (la probabilité d'avoir une différence de moyenne au moins aussi extrème que nos données par rapport à l'hypothèse nulle). Elle es très faible, inférieure à 5%, car l'écart de moyennes important du cas 1 est assez extraordinaire sous l'hypothèse de non efficacité du traitement.

```{r, fig.height=3, echo=FALSE}
density <- ggplot(data = df, aes(x=h0)) + geom_density(fill="grey", alpha = 0.2) + 
  theme_pubr() +
  geom_vline(xintercept = observed_no_effect, color="darkred", size=2)+ 
  ggtitle("Statistique de tests du cas 2 comparée à la distribution nulle")
...
Ici, la p-value représentée par l'aire colorée a une valeur plus forte, suppérieure à notre seuil de 5%, donc on ne pourra pas rejeter l'hypothèse que le traitement n'a pas d'effet.
Si on rajoute une p-value aux graphes présentés plus haut, on obtient :

```{r, fig.width=4, echo=FALSE}
ggplot(biomasse, aes(x=Traitement, y=Biomasse)) + 
  geom_boxplot(color="darkblue") + xlab("")+
  geom_jitter(size=3)+theme_pubr() + stat_compare_means(method = "wilcox.test") + ggtitle("Cas 1")

ggplot(biomasse_no_effect, aes(x=Traitement, y=Biomasse)) + 
  geom_boxplot(color="darkred") + 
  geom_jitter(size=3)+xlab("")+
  theme_pubr() + stat_compare_means(method = "wilcox.test")+ ggtitle("Cas 2")
```

**Le cas 1 montre une différence significative due à l'effet du traitement sur la biomasse, alors que le cas 2 non.**
...
Suivant le type des données et les hypothèses que l'on peut vérifier sur les données, il faut choisir un test statistique adapté.

**Cas d'une variable quantitative et d'une variable qualitative**:

+ Si on a deux groupes pour lesquels on a mesuré une grandeur quantitative, on peut réaliser un test de comparaison de moyennes. Son hypothèse nulle est que les moyennes des deux groupes sont égales.
...
Dans ce cas là, on peut tester si le coefficent de corrélation calculé entre ces deux variables est significatif ou non. 
La p-value de ce test permet de rejeter ou non l'hypothèse nulle, qui est l'absence de corrélation entre les deux variables.

*Note: Il existe les coeffcients de corrélation de `pearson` (paramétrique, pour des données normales ou de grande taille) ou de `spearman` (pour des données non normales et avec une faible taille d'échantillon)*. Il convient d'utiliser la méthode la plus adaptée en fonction des données.
